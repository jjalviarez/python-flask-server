...
def home():
    ...
    return render_template('home.html', logged=True, nickname=session['user_name'], friends=session['friends'], messages=messages,
...
@app.route('/friend_messages', methods=['GET', 'POST'])
def message_amigo():
    """Mensajes de solo un amigo seleccionado"""
    if 'user_name' not in session:
        return process_error("you must be logged to use the app / debe registrarse antes de usar la aplicacion",
                             url_for("login"))
    if request.method == 'POST' and request.form['email'] != "":
        user= request.form.get("email", None)
        if user == 'ALL':
            return jsonify(sorted(get_friends_messages_with_authors(), key=lambda x: x[1]))
        else:
            return jsonify(load_messages_from_user(user))
